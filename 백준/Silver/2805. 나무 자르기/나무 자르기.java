...
- 내가 전에 짰던 코드를 수정하며 깨달은 것은 여기는 결과값을 따로 구하고 출력하는 것이 아니라 max 값이 결과값이기도 하다.
- 즉 자르는 위치를 최대한 죄여가며 내리고 내려서 딱 M만큼 가져갈 수 있게 되는 max 높이 그자체가 답이 되기 때문에 따로 벼수를 둘 필요가 없는 것
- 추후에 까먹을 때쯤 다시 풀어봐야겠다
*/
