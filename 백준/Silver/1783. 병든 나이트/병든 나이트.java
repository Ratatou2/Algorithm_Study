...
[보완점]
- 아 아래 문제 조건 때문에 더 헷갈렸음
    - 병든 나이트의 이동 횟수가 4번보다 적지 않다면, 이동 방법을 모두 한 번씩 사용해야 한다.
    - 이동 횟수가 4번보다 적은 경우(방문한 칸이 5개 미만)에는 이동 방법에 대한 제약이 없다.
- 이게 참 불친절한 설명인게;;
    - 적지 않다면(= 4칸 이상이라면), 이동 방법을 모두 한번씩 사용해야한다(= 이동 방법을 '최소한 한번씩'은 사용해야한다)
    - 이동 횟수가 4보다 적은 경우는 그냥 제약 없음
- 이란 뜻임
- 그냥 한번 쭉 읽으면 이동횟수 4이상부터는 4가지 이동횟수는 무조건 한사이클로 묶어서 써야한다고 읽혀서 계속 M-2가 이해가 안갔음;;;
- ㅎ.. 문제 수정이 필요하지 않나 싶네요... (근데 또 나만 헷갈린걸 수도 헷)
*/
