...
[구현방법]
- 정렬한 다음에 포인터 이동하듯이 이동하며 갑옷을 만드는데 필요한 크기의 재료를 찾는 것이 빠를듯하다
...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        int[] ingredient = new int[N];  // 갑옷 재료 저장할 배열
        for (int i = 0; i < N; i++) {
        ...
        for (int i = 0; i < N; i++) {
            if (endIndex <= startIndex) break;  // 교차했거나, 지나쳤다는 의미는 이미 다 시도했단 뜻이다

            int ingreSum = ingredient[startIndex] + ingredient[endIndex];
