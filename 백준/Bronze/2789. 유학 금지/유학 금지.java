...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        // CAMBRIDGE 철자를 미리 표기해둘 배열
        boolean[] isCamebridge = new boolean[26];
        String camebridge = "CAMBRIDGE";

        // CAMBRIDGE에 해당하는 철자는 true
        for (char temp : camebridge.toCharArray()) {
            isCamebridge[temp - 'A'] = true;
        }

        // input 받고 철자 하나씩 떼서 CAMBRIDGE에 포함되는지 체크
        String input = br.readLine();
        StringBuilder sb = new StringBuilder();
        for (char temp : input.toCharArray()) {
            if(isCamebridge[temp - 'A']) continue;
