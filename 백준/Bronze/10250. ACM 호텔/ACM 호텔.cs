...
다만 걷는 거리가 같을 때에는 아래층의 방을 더 선호한다. 예를 들면 102 호 방보다는 301 호 방을 더 선호하는데, 102 호는 거리 2 만큼 걸어야 하지만 301 호는 거리 1 만큼만 걸으면 되기 때문이다. 
...
[구현방법]
- 한마디로 인풋을 받은 N번째 손님은 H로 나눠서 몫과 나머지를 계산하면 된다.
- 몫은 호수가 될 것이고, 나머지는 층수가 될 것이다

[보완점]
- N % H == 0일 때는 H층이 되어야 하지만 지금 로직은 0층으로 출력
- 또한 이 때는 호수조차 +1이 되면 안됨
*/
...
class Program
{
    public static void Main() {
        ...
        for (int test = 1; test <= T; test++) {
            ...
            sb.Append(floorNum).Append(roomNum < 10 ? "0" + roomNum : roomNum).Append("\n");
        }
